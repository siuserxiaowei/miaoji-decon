...
name: miaoji-decon
description: |
  妙记拆解 · 道法术器。监控飞书妙记，发现新录音后把会议纪要拆成「道·法·术·器·人/关系·信号/钩子·金句」七维结构（嫁接三标签/价值决策树/稳定ID/交叉引用），并同时归档到 Obsidian、飞书多维表格「道法术器拆解库」与飞书在线文档，最后推送 GitHub 并回执在线链接。支持每天定时自动扫描，也支持手动处理单条妙记。触发方式：/妙记拆解、/道法术器、/dfsq、/拆解妙记、/会议纪要拆解。
---
...
完整模板见 `references/template.md`。骨架七维 + 看板 + 关系图 + 映射表：

```text
📊 拆解看板        ← 1屏概览：各维度条数/高价值数/今天最该记住的1件事
🌌 道   本质·底层判断·概念释义        每条带 [来源][价值][时间] + ID(D-n)
🧭 法   策略·可迁移方法论                                      (F-n)
🛠 术   具体动作·技巧·SOP                                      (S-n)
🧰 器   工具·资源·数据·人脉渠道                                (Q-n)
👤 人/关系  谁说的·立场·潜台词·对我的意义                       (P-n)
📡 信号/钩子  值得追的线索 + 待办(带完成判据)                   (X-n)
💬 金句  讲者原话(保留原措辞)                                   (Y-n)
🔗 资产关系图(交叉引用≥3条时, mermaid)
🗺 五类资产映射表(config.emit_asset_map=true 时输出)
```

**三条铁律（继承 situk-yangtao 的诚实纪律）：**

1. 每条强制带三标签：`[来源:讲者讲|我引申|他人问]` `[价值:高|中|低]` `[时间:HH:MM:SS]`。
2. 价值评级走三步决策树（见 `references/template.md`）：行动判定→杠杆判定→认知判定。全篇校准 `[价值:高]`≤30%、`[价值:低]`≥20%，超标重筛。
3. 严格区分"讲者讲 vs 我引申"，金句保留原话措辞，无逐字稿不编造。
