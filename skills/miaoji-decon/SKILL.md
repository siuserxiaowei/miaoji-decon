...
产品级交付标准：miaoji-decon 交付的不是会议摘要，而是能被传播、复用、执行的认知资产。每场会必须回答五个问题：

1. 这场会的核心判断是什么？
2. 这个判断成立的边界是什么？
3. 它能迁移到哪里，用什么模型/漏斗/地图/反例表达？
4. 明天怎么验证，尤其 7 天内怎么做出硬信号？
5. 哪些是事实、同源转述、内部观察、分析推断和建议，各自可信度多高？

低增量交付要主动升级：录音总结 → 一句核心命题；章节概要 → 隐含假设和适用边界；金句摘录 → 模型、漏斗、地图和反例；待办事项 → 7 天验证实验。

五问质量门禁（写完必须自检，未过则重写）：生成正文前，先做一张内部工作卡，回答“核心判断 / 成立边界 / 迁移结构 / 7 天硬信号 / 证据等级”。这张卡不必原样输出，但必须支撑正文。
...
- "这几份材料其实是同一场会，帮没参加的人做深度学习报告"
- "先做证据审计，再按道法术器势给结论、分歧和行动计划"
- "调用 20 个 Agent 拆解，但要如实写清谁完成、谁中断、谁补做"
...
- 使用 `source_mode: source-pack`，列出 `source_families`、统一时间码基准和已知偏移。

不要因为材料多就自动调用多 Agent。只有用户明确要求，或运行环境规则允许且任务确实可独立拆分时才并行；完整规则见 `references/evidence-protocol.md`。
...
🧾 材料与证据口径          ← 来源家族、原始/同源/独立/推断、时间基准
🚦 可信度总览              ← 高/中/低可信，明确什么能传播
🎯 这篇真正能学什么        ← 这场会练的是什么眼力
...
**六条铁律（继承 situk-yangtao 的诚实纪律，并吸收近期深度拆解的证据规则）：**

1. 严格区分"讲者讲 vs 我引申"，无逐字稿支撑处不编造。
2. 数字/判断带销售口径的，标注存疑、提示独立核实。
3. AI 纪要、转载、平台转写与原文同源时，不按多个独立信源计数。
...
### Source-Pack 分支（多材料深度拆解）

1. 读完全部指定材料，记录读取成功/失败项，失败项不假装已读。
